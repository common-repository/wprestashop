...
<link rel="stylesheet" type="text/css" href="<?php echo plugins_url( 'wprestashop/dlf/styles.css') ?>" />
<script type="text/javascript" src="<?php echo plugins_url( 'wprestashop/js/jquery-1.6.2.min.js') ?>"></script>
<script type="text/javascript">
    jQuery(document).ready(function(){
        jQuery('span.select').click(function(){
            var attr = jQuery(this).attr('alt');
            parent.jQuery('input#wsite').val(attr);
...
<body>
<div id="container">
  <div id="breadcrumbs"> <a href="<?php echo strip_tags($_SERVER['PHP_SELF']).'?token='.$token;?>">home</a> 
  <?php
  	...
  	if(($bsize = sizeof($breadcrumbs))>0) {
  		...
  		for($bi=0;$bi<($bsize-1);$bi++) {
			...
			echo ' &gt; <a href="'.strip_tags($_SERVER['PHP_SELF']).'?token='.$token.'&dir='.urlencode($sofar).'">'.$breadcrumbs[$bi].'</a>';
		}
  	}
  ...
  <div id="listingcontainer">
	...
	?>
	<div>
	<a href="<?php echo strip_tags($_SERVER['PHP_SELF']).'?token='.$token.'&dir='.urlencode($dotdotdir);?>" class="<?php echo $class;?>"><img src="<?php echo $includeurl; ?>dlf/dirup.png" alt="Folder" /><strong>..</strong> <em>&nbsp;</em>&nbsp;</a></div>
	<?php
		if($class=='b') $class='w';
		else $class = 'b';
	}
	...
	for($i=0;$i<$arsize;$i++) {
	?>
		<div><a href="<?php echo strip_tags($_SERVER['PHP_SELF']).'?token='.$token.'&dir='.urlencode(str_replace($startdir,'',$leadon).$dirs[$i]);?>" class="<?php echo $class;?>">
				<strong><?php echo $dirs[$i];?></strong> &nbsp;
			</a>
			<span class='select' alt="<?php echo str_replace($startdir,'',$leadon).$dirs[$i] ?>">Select</span>
		</div>
	<?php
		if($class=='b') $class='w';
		else $class = 'b';	
